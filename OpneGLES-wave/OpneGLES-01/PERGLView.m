...
@property (nonatomic, assign) GLuint transformMatrix;
...
- (void)setupShader {
    ...
    self.transformMatrix = glGetUniformLocation(self.shader.programHandle, "transform");
...
//    [self setupData];
    ...
    [matrix translateByX:0.5];
    [matrix translateByY:0.5];
    [matrix rotateByZ:cos([date timeIntervalSinceReferenceDate])];



    glUniformMatrix4fv(self.transformMatrix, 1, GL_FALSE, matrix.glMatrix);
    ...
    CC3GLMatrix *matrix1 = [CC3GLMatrix identity];
    [matrix1 translateByX:-0.5];
    [matrix1 translateByY:-0.5];
    glUniformMatrix4fv(self.transformMatrix, 1, GL_FALSE, matrix1.glMatrix);
    
    glUseProgram(self.shader.programHandle);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, _indexBuffer);
//    glDrawElements(GL_TRIANGLES, sizeof(indices)/sizeof(indices[0]), GL_UNSIGNED_BYTE, 0);

    glDrawArrays(GL_LINE_STRIP, 0, sizeof(vertices)/sizeof(SceneVertex));
...
//    vertices = malloc(sizeof(SceneVertex) * width);
    NSInteger heith = 1;
    for (NSInteger i = 0; i < width; i++) {
        
        
//        if (i%31 == 0) {
//            heith = arc4random()%5/5.0+0.5;
//        }
        
        SceneVertex *point = vertices + i;
        point->Position[0] = (1.0*i/width - 0.5)*2;

        point->Position[1] = fabsf(sin(i/100.0)) * heith;
        point->Position[2] = 0;
